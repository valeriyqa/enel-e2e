...
namespace JuiceBoxQA.UITests.Helpers
{
    public class JBElements
    {
        ...
        public static void Submit(IWebDriver driver, By by)
        {
            try
            {
            ...
            {
                ExtentTest test = ScenarioContext.Current.Get<ExtentTest>();
                test.Error("Element identified by " + by.ToString() + " not submitable after " + Constants.DefaultTimeout.ToString() + " seconds", MediaEntityBuilder.CreateScreenCaptureFromPath(ReportingMethods.CreateScreenshot(driver)).Build());
                Assert.Fail();
            }
        }
